...
`docs/promptprojeto.md` por inteiro. Ele é a fonte de verdade do
...
- Antes de implementar algo novo, explicar objetivo e decisões técnicas;
  ...
  antes de escolher.
- Ao finalizar uma sprint, atualizar `docs/promptprojeto.md` (seção de
...
- Não há Maven local nem `mvnw` — `mvn` roda via container
  ...
  seção de validação de cada sprint em `docs/promptprojeto.md` para o
  comando exato usado.
